  ...
  class AddCaseRequestor
    ...
    def build_request
      Nokogiri::XML::Builder.new(encoding: 'UTF-8') do |xml|
        xml.__send__('ns7:CaseAddRQ', NAMESPACES) {
    ...
    def generate_category_of_law(xml)
      ...
      xml.__send__('ns2:RequestedAmount', @legal_aid_application.requested_amount)
    end
    ...
    def generate_vehicles_entity(xml, sequence_no)
      ...
      vehicles.each_with_index { |vehicle, i| generate_vehicle_instance(xml, vehicle, i) }
    end

    def generate_vehicle_instance(xml, vehicle, index)
      xml.__send__('ns0:Instances') do
    ...
    def generate_means_proceeding_entity(xml, sequence_no)
      ...
      proceedings.reverse.each { |proceeding| generate_means_proceeding_instance(xml, proceeding) }
    end
    ...
    def generate_merits_proceeding_entity(xml, sequence_no)
      xml.__send__('ns0:SequenceNumber', sequence_no)
      xml.__send__('ns0:EntityName', 'PROCEEDING')
      @legal_aid_application.proceeding_types.reverse.each { |p| generate_merits_proceeding_instance(xml, p) }
    end
    ...
    def extract_raw_value(config, options)
      if config[:value] == true || config[:value] == false
        ...
        is_method_name?(config[:value]) ? get_attr_value(config[:value], options) : config[:value]
      end
    end

    def is_method_name?(str)
